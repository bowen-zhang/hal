...
CastStatus = namedtuple('CastStatus', [
    'is_active_input', 'is_stand_by', 'volume_level', 'volume_muted', 'app_id',
    'display_name', 'namespaces', 'session_id', 'transport_id', 'status_text'
])
...
class SocketClient(threading.Thread):
  ...
  def __init__(self,
               host,
               port=None,
               cast_type=types.CastType.CHROMECAST,
               **kwargs):
    ...
    self.media_controller = media.MediaController()
    ...
    if self.socket in can_read and not self._force_recon:
      try:
        ...
        if self.stop.is_set():
          ...
          self.logger.error("Interruption caught without being stopped: %s",
                            exc)
        return 1
  ...
  def _route_message(self, message, data):
    ...
    if message.namespace in self._handlers:
      ...
      if message.namespace != NS_HEARTBEAT:
        self.logger.debug("Received: %s", _message_to_string(message, data))
      ...
      try:
        ...
        if not handled:
          if data.get(REQUEST_ID) not in self._request_callbacks:
            self.logger.debug("Message unhandled: %s",
                              _message_to_string(message, data))
      except Exception:  # pylint: disable=broad-except
        self.logger.exception(("Exception caught while sending message to "
                               "controller %s: %s"),
                              type(self._handlers[message.namespace]).__name__,
                              _message_to_string(message, data))

    else:
      self.logger.debug("Received unknown namespace: %s",
                        _message_to_string(message, data))
  ...
  def _report_connection_status(self, status):
    ...
    for listener in self._connection_listeners:
      try:
        self.logger.debug("connection listener: %x (%s)", id(listener),
    ...
    if namespace not in self.app_namespaces:
      raise UnsupportedNamespace(
          ("Namespace {} is not supported by current app. "
           "Supported are {}").format(namespace, ", ".join(
               self.app_namespaces)))
...
class ConnectionController(base.BaseController):
...
class HeartbeatController(base.BaseController):
...
class ReceiverController(base.BaseController):
  """
  ...
  def __init__(self, cast_type=types.CastType.CHROMECAST, blocking=True):
  ...
  def launch_app(self, app_id, force_launch=False, callback_function=False):
    ...
    if not force_launch and self.app_id is None:
      self.update_status(
          lambda response: self._send_launch_message(app_id, force_launch, callback_function)
      )
    else:
      self._send_launch_message(app_id, force_launch, callback_function)

  def _send_launch_message(self,
                           app_id,
                           force_launch=False,
                           callback_function=False):
  ...
  def _block_till_launched(self, app_id):
    if self.blocking:
      ...
      if self.launch_failure:
        raise LaunchError("Failed to launch app: {}, Reason: {}".format(
  ...
  def _parse_status(data, cast_type):
    ...
    is_audio = cast_type in (types.CastType.AUDIO, types.CastType.GROUP)
    ...
    status = CastStatus(
        ...
        app_data.get(APP_ID), app_data.get('displayName'),
        [item['name'] for item in app_data.get('namespaces', [])],
        app_data.get(SESSION_ID), app_data.get('transportId'),
