...
class SHT1x(pattern.Logger):
  ...
  def __init__(self,
               ...
               *args,
               **kwargs):
    super(SHT1x, self).__init__(self, *args, **kwargs)
    self.data_pin = data_pin
    self.sck_pin = sck_pin
    self.gpio_mode = gpio_mode
    self.vdd = self.VDD.get(vdd.upper(), self.VDD['3.5V'])
    self._resolution = self.RESOLUTION.get(resolution.capitalize(),
    ...
    self.logger.info(
        ...
        'Resolution: {4}\nHeater: {5}\nOTP no reload: {6}\nCRC check: {7}'
        .format(self.data_pin, self.sck_pin, GPIO_FUNCS[gpio_mode], self.vdd,
                resolution, self._heater, self._otp_no_reload, self.crc_check))
  ...
  def initialize_sensor(self):
    ...
    self.logger.info(
        'Initializing sensor using bit mask: {0:08b}'.format(mask))
  ...
  def read_temperature(self):
    ...
    self.temperature_celsius = round(
        raw_temperature * COF.D2_SO_C[self._resolution[0]] +
        COF.D1_VDD_C[self.vdd], 2)
    self.temperature_fahrenheit = round(
        raw_temperature * COF.D2_SO_F[self._resolution[0]] +
        COF.D1_VDD_F[self.vdd], 2)

    self.logger.info('Temperature: {0}*C [{1}*F]'.format(
        self.temperature_celsius, self.temperature_fahrenheit))
  ...
  def read_humidity(self, temperature=None):
    ...
    linear_humidity = COF.C1_SO[self._resolution[1]] + (
        COF.C2_SO[self._resolution[1]] * raw_humidity) + (
            COF.C3_SO[self._resolution[1]] * raw_humidity**2)

    self.humidity = round(
        (temperature - 25) *
        (COF.T1_SO[self._resolution[1]] + COF.T2_SO[self._resolution[1]] *
         raw_humidity) + linear_humidity, 2)
  ...
  def _wait_for_result(self):
    ...
    if data_ready == GPIO.HIGH:
      raise SHT1xError(
          'Sensor has not completed measurement after max time allotment.\n{0}'.
          format(self))
  ...
  def _get_ack(self, command_name):
    ...
    ack = GPIO.input(self.data_pin)
    self.logger.info('Command {0} [{1:08b}] acknowledged: {1}'.format(
        command_name, self._command, ack))
    if ack == GPIO.HIGH:
      message = 'SHT1x failed to properly receive command [{0} - {1:08b}]'.format(
  ...
  def read_status_register(self):
    ...
    self.logger.debug(
        "Status Register read: {0:08b}".format(self._status_register))
    ...
    self.logger.info(
        "Read Status Register: {0:08b}".format(self._status_register))
  ...
  def _validate_crc(self, data, measurement=True):
    ...
    self.logger.info(
        'CRC calculated value (reversed): {0:08b}'.format(crc_final_reversed))
  ...
  def __str__(self):
    ...
    return 'Temperature: {0}*C [{1}*F]\nRelative Humidity: {2}%\nDew Point: {3}*C\n'.format(
        celsius, fahrenheit, humidity, dew_point)
